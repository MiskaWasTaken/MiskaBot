...
module.exports = {
    ...
    tictactoe: async function (message, options = []) {
        ...
        async function tictactoe(m) {
            ...
            if (won["<:O_:863314110560993340>"] != false) return m.edit({ content: `<@!${Args.userid}> (${o_emoji}) won.. That was a nice game.`, components: [] })
            ...
            if (won["<:X_:863314044781723668>"] != false) return m.edit({ content: `<@!${Args.userid}> (${x_emoji}) won.. That was a nice game.`, components: [] })
            ...
            collector.on('end', collected => {
                if (collected.size == 0) m.edit({ content: `<@!${Args.userid}> didn\'t react in time! (30s)` })
            });
        }
    },
    ...
    embedPages: async function (client, message, pages, style = []) {
        ...
        var currentPage = 0;
        var m = await message.reply({ embeds: [pages[0]],  components: [pageMovingButtons] });
